    ...
    <MainLayout>
      <div className="animate-fade-in">
        <div className="flex justify-between items-center mb-6">
          <h1 className="text-3xl font-bold rtl arabic text-emerald-700">سجل الحضور</h1>
          <Button 
            className="bg-emerald-600 hover:bg-emerald-700 text-white rtl arabic shadow-sm transition-all duration-300"
          >
        ...
        <Card className="mb-6 border-emerald-100 shadow-sm hover:shadow-md transition-all duration-300">
          <CardHeader className="pb-3 bg-gradient-to-r from-emerald-50 to-transparent border-b border-emerald-100">
            <div className="flex justify-between items-center">
              <CardTitle className="text-lg rtl arabic text-emerald-700">يوم الحضور</CardTitle>
          ...
          <CardContent>
            <div className="text-center mb-4">
              <h2 className="text-xl font-bold rtl arabic text-emerald-700">{formatDate(selectedDate)}</h2>
            </div>
          </CardContent>
        </Card>

        <Tabs defaultValue="all" className="mb-6" onValueChange={setActiveTab}>
          <TabsList className="grid grid-cols-2 sm:grid-cols-5 mb-4 bg-emerald-50/50 p-1">
            <TabsTrigger 
              value="all" 
              className="arabic data-[state=active]:bg-emerald-600 data-[state=active]:text-white"
            >
              الكل
            </TabsTrigger>
            <TabsTrigger 
          ...
          <div className="overflow-x-auto">
            <table className="w-full">
              <thead className="bg-emerald-50/50">
                <tr>
                  <th className="py-3 px-4 text-right rtl arabic text-emerald-700">الاسم</th>
                  <th className="py-3 px-4 text-right rtl arabic text-emerald-700">النوع</th>
                  <th className="py-3 px-4 text-right rtl arabic text-emerald-700">الحضور</th>
                  <th className="py-3 px-4 text-right rtl arabic text-emerald-700">الإجراءات</th>
                </tr>
              </thead>
              <tbody>
                {getFilteredAttendance().map((member, index) => (
                  ...
                  >
                    <td className="py-3 px-4 rtl arabic text-emerald-700">{member.name}</td>
                    <td className="py-3 px-4 rtl arabic text-emerald-600">
